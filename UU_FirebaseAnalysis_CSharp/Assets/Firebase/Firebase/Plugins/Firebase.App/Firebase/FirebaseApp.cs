...
namespace Firebase
{
	public sealed class FirebaseApp : IDisposable
	{
		...
		[CompilerGenerated]
		private static FirebaseApp.LogMessageDelegate f__mgcache0;
		...
		internal static object WeakReferenceGetTarget(WeakReference weakReference)
		{
			if (weakReference != null)
			{
				try
				{
					object result = weakReference.Target;
					return result;
				}
		...
		private static bool InitializeCrashlyticsIfPresent()
		{
			try
			{
				...
				MethodInfo method = type.GetMethod("Initialize", BindingFlags.Public | BindingFlags.Static);
				if (method == null)
				{
