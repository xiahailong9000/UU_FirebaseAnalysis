			...
			try
			{
				...
				for (int i = 0; i < assemblies.Length; i++)
				{
					...
					if (assembly.GetName().Name.Equals("Mono.Security"))
					{
						...
						if (type != null)
						{
							FieldInfo field = type.GetField("_userPath", BindingFlags.NonPublic | BindingFlags.Static);
							if (field != null)
							{
								field.SetValue(null, null);
							}
							FieldInfo field2 = type.GetField("_userStore", BindingFlags.NonPublic | BindingFlags.Static);
							if (field2 != null)
							{
